...
describe("ObligationsService", () => {
  ...
  describe("calculateTrafficLight", () => {
    ...
    it("should return GREEN for COMPLETED status", () => {
      ...
      const result = service.calculateTrafficLight(
        dueDate,
        "COMPLETED" as any,
        thresholdYellow,
        thresholdRed,
  ...
  describe("create", () => {
    ...
    it("should create an obligation successfully", async () => {
      ...
      mockPrismaService.userOrg.findFirst.mockResolvedValue({
        userId: "user-123",
        organizationId,
      });
    ...
    it("should throw BadRequestException if location does not belong to organization", async () => {
      ...
      await expect(
        service.create(organizationId, dtoWithLocation),
      ).rejects.toThrow(
  ...
  describe("updateStatus", () => {
    ...
    it("should throw BadRequestException when completing without required evidence", async () => {
      ...
      await expect(
        service.updateStatus(organizationId, obligationId, "COMPLETED" as any),
      ).rejects.toThrow(
