...
describe('NotificationsService', () => {
  ...
  describe('notifyUpcomingObligations', () => {
    it('should send notifications to obligation owners', async () => {
      const mockOrganizations = [
        { id: 'org-1', name: 'Org 1', active: true, thresholdYellowDays: 15, thresholdRedDays: 7 },
      ];
      ...
      expect(mockEmailService.sendUpcomingObligationsEmail).toHaveBeenCalledWith(
        'owner@test.com',
        'Owner Name',
        'Org 1',
        expect.any(Array),
  ...
  describe('notifyOverdueObligations', () => {
    it('should send notifications to organization owners', async () => {
      ...
      mockPrismaService.organization.findMany.mockResolvedValue(mockOrganizations);
      mockPrismaService.obligation.findMany.mockResolvedValue(mockOverdueObligations);
    ...
    it('should skip organizations with no overdue obligations', async () => {
      ...
      expect(result).toBe(0);
      expect(mockEmailService.sendOverdueObligationsEmail).not.toHaveBeenCalled();
    });
  });
