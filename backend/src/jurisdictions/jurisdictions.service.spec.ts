...
describe("JurisdictionsService", () => {
  ...
  describe("findAllActive", () => {
    it("should return only active jurisdictions", async () => {
      ...
      mockPrismaService.jurisdiction.findMany.mockResolvedValue(
        mockJurisdictions,
      );
  ...
  describe("findByCode", () => {
    ...
    it("should throw NotFoundException if code not found", async () => {
      ...
      await expect(service.findByCode("non-existent-code")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("update", () => {
    it("should update jurisdiction", async () => {
      ...
      const result = await service.update("juris-123", {
        name: "Rosario Updated",
      });
